...
async def create(data: EmploymentHistoryCreate, controller: EmploymentHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("employee.manage"))):
...
async def get_by_id(id: str, controller: EmploymentHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("employee.read"))):
...
async def update(id: str, data: EmploymentHistoryUpdate, controller: EmploymentHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("employee.manage"))):
    doc = await controller.get_by_id(id)
    await authz.validate_resource_employee(doc.employeeId)
...
@router.delete("/{id}")
async def delete(id: str, controller: EmploymentHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("employee.manage"))):
    doc = await controller.get_by_id(id)
    await authz.validate_resource_employee(doc.employeeId)
