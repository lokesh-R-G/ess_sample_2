...
async def create(data: EmployeeExitCreate, controller: EmployeeExitController = Depends(get_controller), user: dict = Depends(get_current_user)):
...
async def get_by_id(id: str, controller: EmployeeExitController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.get_by_id(id)

@router.put("/{id}", response_model=EmployeeExitResponse)
async def update(id: str, data: EmployeeExitUpdate, controller: EmployeeExitController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.update(id, data, user.get("empId"))

@router.delete("/{id}")
async def delete(id: str, controller: EmployeeExitController = Depends(get_controller), user: dict = Depends(get_current_user)):
