...
from ..controllers.employee_document_controller import EmployeeDocumentController
...
async def create(data: EmployeeDocumentCreate, controller: EmployeeDocumentController = Depends(get_controller), user: dict = Depends(get_current_user)):
...
async def get_by_id(id: str, controller: EmployeeDocumentController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.get_by_id(id)

@router.put("/{id}", response_model=EmployeeDocumentResponse)
async def update(id: str, data: EmployeeDocumentUpdate, controller: EmployeeDocumentController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.update(id, data, user.get("empId"))

@router.delete("/{id}")
async def delete(id: str, controller: EmployeeDocumentController = Depends(get_controller), user: dict = Depends(get_current_user)):
