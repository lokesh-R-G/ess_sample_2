...
async def create(data: EmployeeConfirmationCreate, controller: EmployeeConfirmationController = Depends(get_controller), user: dict = Depends(get_current_user)):
...
async def get_by_id(id: str, controller: EmployeeConfirmationController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.get_by_id(id)

@router.put("/{id}", response_model=EmployeeConfirmationResponse)
async def update(id: str, data: EmployeeConfirmationUpdate, controller: EmployeeConfirmationController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.update(id, data, user.get("empId"))

@router.delete("/{id}")
async def delete(id: str, controller: EmployeeConfirmationController = Depends(get_controller), user: dict = Depends(get_current_user)):
