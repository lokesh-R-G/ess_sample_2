...
class GracePolicyService:
    def __init__(self, db: AsyncIOMotorDatabase):
        self.db = db
        self.repo = GracePolicyRepository(db)
        self.validator = GracePolicyValidator(db)
        
    async def create(self, data: GracePolicyCreate, user_id: str = None) -> GracePolicyModel:
    ...
    async def get_by_id(self, id: str) -> Optional[GracePolicyModel]:
        return await self.repo.get_by_id(id)
        
    async def update(self, id: str, data: GracePolicyUpdate, user_id: str = None) -> Optional[GracePolicyModel]:
