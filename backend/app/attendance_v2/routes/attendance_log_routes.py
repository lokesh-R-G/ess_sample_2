...
def get_controller(db = Depends(get_database)) -> AttendanceLogController:
    return AttendanceLogController(db)

@router.post("/", response_model=AttendanceLogResponse)
async def create(data: AttendanceLogCreate, controller: AttendanceLogController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
...
async def get_all(
    ...
    controller: AttendanceLogController = Depends(get_controller),
...
@router.get("/{id}", response_model=AttendanceLogResponse)
async def get_by_id(id: str, controller: AttendanceLogController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.read"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
    return doc

@router.put("/{id}", response_model=AttendanceLogResponse)
async def update(id: str, data: AttendanceLogUpdate, controller: AttendanceLogController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
...
@router.delete("/{id}")
async def delete(id: str, controller: AttendanceLogController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
