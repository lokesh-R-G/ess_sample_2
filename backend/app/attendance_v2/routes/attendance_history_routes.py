...
from app.attendance_v2.controllers.attendance_history_controller import AttendanceHistoryController
...
async def create(data: AttendanceHistoryCreate, controller: AttendanceHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
...
async def get_by_id(id: str, controller: AttendanceHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.read"))):
...
async def update(id: str, data: AttendanceHistoryUpdate, controller: AttendanceHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
...
@router.delete("/{id}")
async def delete(id: str, controller: AttendanceHistoryController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
