...
from app.attendance_v2.controllers.attendance_calculation_controller import AttendanceCalculationController
...
async def create(data: AttendanceCalculationCreate, controller: AttendanceCalculationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
...
async def get_by_id(id: str, controller: AttendanceCalculationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.read"))):
...
async def update(id: str, data: AttendanceCalculationUpdate, controller: AttendanceCalculationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
...
@router.delete("/{id}")
async def delete(id: str, controller: AttendanceCalculationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("attendance.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
