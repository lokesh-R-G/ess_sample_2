...
async def get_by_id(id: str, controller: BranchController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.get_by_id(id)

@router.put("/{id}", response_model=BranchResponse)
async def update(id: str, data: BranchUpdate, controller: BranchController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.update(id, data, user.get("empId"))

@router.delete("/{id}")
async def delete(id: str, controller: BranchController = Depends(get_controller), user: dict = Depends(get_current_user)):
