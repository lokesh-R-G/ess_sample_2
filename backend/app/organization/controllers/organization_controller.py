from typing import List, Optional
from motor.motor_asyncio import AsyncIOMotorDatabase
from fastapi import HTTPException
from app.organization.services.organization_service import OrganizationService
from app.organization.schemas.organization import OrganizationCreate, OrganizationUpdate, OrganizationResponse
from app.organization.models.organization import OrganizationModel

class OrganizationController:
    def __init__(self, db: AsyncIOMotorDatabase):
        self.service = OrganizationService(db)
        
    async def create(self, data: OrganizationCreate, user_id: str) -> OrganizationModel:
        return await self.service.create(data, user_id)
        
    async def get_all(self, query: dict, skip: int, limit: int, search: str) -> dict:
        return await self.service.get_all(query, skip, limit, search)
        
    async def get_by_id(self, id: str) -> OrganizationModel:
        doc = await self.service.get_by_id(id)
        if not doc:
            raise HTTPException(status_code=404, detail="Organization not found")
        return doc
        
    async def update(self, id: str, data: OrganizationUpdate, user_id: str) -> OrganizationModel:
        doc = await self.service.update(id, data, user_id)
        if not doc:
            raise HTTPException(status_code=404, detail="Organization not found")
        return doc
        
    async def delete(self, id: str, user_id: str) -> dict:
        success = await self.service.delete(id, user_id)
        if not success:
            raise HTTPException(status_code=404, detail="Organization not found")
        return {"message": "Organization archived successfully"}
