...
async def create(data: LeaveReservationCreate, controller: LeaveReservationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.manage"))):
...
async def get_by_id(id: str, controller: LeaveReservationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.read"))):
...
async def update(id: str, data: LeaveReservationUpdate, controller: LeaveReservationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
...
@router.delete("/{id}")
async def delete(id: str, controller: LeaveReservationController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
