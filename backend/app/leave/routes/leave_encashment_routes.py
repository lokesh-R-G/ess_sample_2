...
async def create(data: LeaveEncashmentCreate, controller: LeaveEncashmentController = Depends(get_controller), user: dict = Depends(get_current_user)):
...
async def get_by_id(id: str, controller: LeaveEncashmentController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.get_by_id(id)

@router.put("/{id}", response_model=LeaveEncashmentResponse)
async def update(id: str, data: LeaveEncashmentUpdate, controller: LeaveEncashmentController = Depends(get_controller), user: dict = Depends(get_current_user)):
    return await controller.update(id, data, user.get("empId"))

@router.delete("/{id}")
async def delete(id: str, controller: LeaveEncashmentController = Depends(get_controller), user: dict = Depends(get_current_user)):
