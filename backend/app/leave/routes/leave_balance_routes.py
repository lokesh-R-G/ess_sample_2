...
def get_controller(db = Depends(get_database)) -> LeaveBalanceController:
    return LeaveBalanceController(db)

@router.post("/", response_model=LeaveBalanceResponse)
async def create(data: LeaveBalanceCreate, controller: LeaveBalanceController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.manage"))):
...
async def get_all(
    ...
    controller: LeaveBalanceController = Depends(get_controller),
...
@router.get("/{id}", response_model=LeaveBalanceResponse)
async def get_by_id(id: str, controller: LeaveBalanceController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.read"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
    return doc

@router.put("/{id}", response_model=LeaveBalanceResponse)
async def update(id: str, data: LeaveBalanceUpdate, controller: LeaveBalanceController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
...
@router.delete("/{id}")
async def delete(id: str, controller: LeaveBalanceController = Depends(get_controller), authz: AuthorizedScope = Depends(authorize("leave.manage"))):
    doc = await controller.get_by_id(id)
    if getattr(doc, "employeeId", None):
        await authz.validate_resource_employee(doc.employeeId)
