...
def app_client(mock_db):
    ...
    from app.attendance_v2.routes.attendance_recalculate_routes import router
    ...
    # We will use AsyncClient in the test directly
    yield test_app
...
async def test_recalculate_auth(app_client, mock_db):
    ...
    async with AsyncClient(transport=ASGITransport(app=app_client), base_url="http://test") as ac:
        resp = await ac.post("/recalculate", json=payload)
        ...
        for role in ["employee", "manager", "accounts"]:
            ...
            assert resp.status_code == 403, f"Expected 403 for {role}, got {resp.status_code}: {resp.json()}"
